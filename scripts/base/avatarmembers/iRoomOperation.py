...
class iRoomOperation(object):
	...
	def createRoom(self, gameRound, roomMode, is_agent):
		DEBUG_MSG("create room,gameRound:{0}, roomMode:{1}".format(gameRound, roomMode))
	...
	def createRoomCallback(self, room, err = None):
		if room:
			...
			if room.is_agent == 1:
				...
				card_cost, diamond_cost = switch.calc_cost(room.roomMode, room.game_round)
				if switch.DEBUG_BASE:
					callback('{"card":99, "diamond":999}'.encode())
				else:
					utility.update_card_diamond(self.accountName, -card_cost, -diamond_cost, callback, "TuiDaoHu RoomID:" +str(self.roomID)) # Reason必须为英文
			else:
				room.reqEnterRoom(self, True)
				self.room = room
				KBEngine.globalData["GameWorld"].addRoom(room)
	...
	def startGame(self, dealer_idx, tile_list, kingTile, diceList, curOldDealNum, gameRound, roomMode):
		if getattr(self, 'client', None):
			self.client.startGame(dealer_idx, tile_list, kingTile, diceList, curOldDealNum, gameRound, roomMode)

	def postOperation(self, idx, aid, tile_list):
		if getattr(self, 'client', None):
			self.client.postOperation(idx, aid, tile_list)

	def postMultiOperation(self, idx_list, aid_list, results, tile_list):
		if getattr(self, 'client', None):
			self.client.postMultiOperation(idx_list, aid_list, results, tile_list)
