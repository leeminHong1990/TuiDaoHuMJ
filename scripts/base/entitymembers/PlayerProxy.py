...
class PlayerProxy(object):
	...
	def exposedKong(self, tile):
		...
		winType = 1		#抢杠
		win_list,results = self.owner.checkOthersWin(self.idx, winType, tile)
		if not win_list:
			# 明杠得分, 只有不被抢杠胡才能得分
			# 放杠
			...
			self.owner.waitForOperation(self.idx, const.OP_EXPOSED_KONG, tile, winType)
	...
	def win(self, multiple, resultList):
		""" 自摸胡 """
		self.win_times += 1
		...
		self.owner.winGame(self.idx, const.OP_WIN, multiple, resultList)
