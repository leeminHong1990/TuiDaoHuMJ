...
class GameRoom(KBEngine.Base, GameObject, iRoomRules):
	...
	def __init__(self):
		...
		#等待玩家确认胡的dict --目前只确认 放炮胡
		self.wait_for_win_dict = {} #{"state": 0,"result":resultList} state 0 未确认 1确认胡 2不胡
	...
	def startGame(self):
		...
		def callback(content):
			...
			try:
				...
				for p in self.players_list:
					if p and p.mb:
						p.mb.startGame(self.dealer_idx, p.tiles, self.kingTile, diceList, self.curOldDealNum, self.game_round, self.roomMode)
				self.beginRound()
			except:
				DEBUG_MSG("consume failed!")
				
		if self.current_round == 1 and self.is_agent == 0:
			card_cost, diamond_cost = switch.calc_cost(self.roomMode, self.game_round)
	...
	def doOperation(self, avt_mb, aid, tile_list):
		...
		if aid == const.OP_DISCARD and self.can_discard(p.tiles, tile):
			...
			resultList = self.can_win(list(p.tiles), 0, idx)
			if sum([i for i in resultList]) > 0:
				DEBUG_MSG("cal_multiple:{0}.".format(str(self.cal_multiple(resultList)), str(self.curOldDealNum)))
				# multiple = self.cal_multiple(resultList) + self.curOldDealNum
	...
	def confirmOperation(self, avt_mb, aid, tile_list):
		...
		if idx not in self.wait_idx_list and len(self.wait_idx_list) > 0:
			DEBUG_MSG("confirmOperation {0},{1}".format(idx, str(self.wait_idx_list)))
			avt_mb.doOperationFailed(const.OP_ERROR_NOT_CURRENT)
			return
		# 确认要操作的牌是否是最后一张被打的牌
	...
	def handleMutiWinAfterCommit(self, tile): # return 是否提交完毕， 是否可以胡
		...
		if not win_list:
			...
			return True, False
		# 有玩家选择胡
		self.processMutiWin(win_list, result_list, formIdxList, tile)
		return True,True

	def confirmPass(self, idx):
		nextIdx = (self.current_idx + 1) % const.ROOM_PLAYER_NUMBER
		nextMb = self.players_list[nextIdx]
		if idx != nextIdx and self.can_chow(nextMb.tiles, self.all_discard_tiles[-1], idx):
	...
	def broadcastMultiOperation(self, idx_list, aid_list, results, tile_list=None):
		for i, p in enumerate(self.players_list):
			if p is not None:
				p.mb.postMultiOperation(idx_list, aid_list, results, tile_list)
	...
	def waitForOperation(self, idx, aid, tile, winType = 2):
		#能不能胡(放炮)
		giveWin = False
		self.wait_idx_list = []
		self.wait_for_win_dict = {}
		player_list, result_list = self.checkOthersWin(idx, winType, tile)
		for i in range(len(player_list)):
			p = player_list[i]
			wait_idx = self.players_list.index(p)
			if aid == const.OP_DISCARD or aid == const.OP_EXPOSED_KONG:
				giveWin = True
				DEBUG_MSG("wait for operation win:{0}".format(wait_idx))
				self.wait_idx_list.append(wait_idx)
				self.wait_op = const.OP_GIVE_WIN
				self.wait_for_win_dict[wait_idx] = {"state": 0, "formIdx": idx, "result":result_list[i]}
				p.mb.waitForOperation(const.OP_GIVE_WIN, [tile,])
		#不能放炮胡
		if not giveWin:
			#能不能碰和杠
			for i, p in enumerate(self.players_list):
	...
	#判断是否可以 放炮/抢杠胡
	def checkOthersWin(self, idx, winType, tile): # winType 1-抢杠 , 2-放炮
		wait_player_list = []
		wait_for_list = []
		for i, p in enumerate(self.players_list):
			if i == idx:
			...
			resultList = self.can_win(tmp, winType, i)
			if sum([i for i in resultList]) > 0:
				wait_player_list.append(p)
				wait_for_list.append(resultList)
		return wait_player_list, wait_for_list

	#放炮/抢杠 胡处理
	def processMutiWin(self, win_list, result_list, formIdx_list, tile):
		""" 处理抢杠胡, 可能有多人胡的情况 """
		# if len(win_list) == 1:
		# 	p = win_list[0]
		# 	p.give_win(tile)
		# 	self.broadcastOperation(p.idx, const.OP_GIVE_WIN, [tile,])
		# 	multiple = self.cal_multiple(result_list[0])
		# 	self.winGame(p.idx, const.OP_GIVE_WIN, multiple, result_list)
		# else:

		win_idx_list = [mem.idx for mem in win_list]
		op_list = [const.OP_GIVE_WIN] * len(win_idx_list)
		self.broadcastMultiOperation(win_idx_list, op_list, result_list, [tile,])

		self.dealer_idx = win_idx_list[random.randint(0, len(win_idx_list)-1)]
		all_mem_tiles = []
		for mem in win_list:
			mem.give_win(tile)
		...
		for i in range(len(win_list)):
			...
			multiple = self.cal_multiple(result_list[i])
			curVal = pow(2,multiple)
			win_list[i].addScore(curVal)
			self.players_list[fromIdx].addScore(-curVal)
			DEBUG_MSG("cal muti score,{0}:{1}, {2}:{3}".format(win_list[i].idx, curVal, fromIdx, -curVal))
	...
	def cal_score(self, idx, aid, lucky_tile = 0, multiple = 0):
		if aid == const.OP_EXPOSED_KONG: # 2 1 1 
			...
			DEBUG_MSG("cal_score:OP_WIN")
			curVal = pow(2,multiple)
			for i, p in enumerate(self.players_list):
				if i != idx:
					 p.addScore(-curVal)
			self.players_list[idx].addScore(curVal * 3)
