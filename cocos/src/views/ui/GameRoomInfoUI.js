...
var GameRoomInfoUI = UIBase.extend({
    ...
    initUI: function () {
        ...
        //局数
        var max_round=this.roominfo_panel.getChildByName("maxRound");
        max_round.setString("局数: "+h1global.entityManager.player().curGameRoom.maxRound.toString());
        //是否放炮胡
        var room_mode=this.roominfo_panel.getChildByName("roomMode");
        if(h1global.entityManager.player().curGameRoom.roomMode.toString()==1){
            room_mode.setString("放炮胡: 是");
        }else{
            room_mode.setString("放炮胡: 否");
        };
