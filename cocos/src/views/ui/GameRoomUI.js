...
var GameRoomUI = UIBase.extend({
	...
	initUI:function(){
		...
		this.update_draw_tile_panel(h1global.entityManager.player().curGameRoom.leftTileNum);

		this.update_roominfo_panel();

		this.update_kingtile_panel();

		this.show_yellow_frame(h1global.entityManager.player().curGameRoom.curPlayerSitNum);
	...
	init_curplayer_panel:function(){
		...
		this.directionList = ["east", "north", "west", "south"];
		var player = h1global.entityManager.player();
		for(var i = 0; i < 4; i++){
			var direct_img = this.cur_player_panel.getChildByName("direct_img" + i.toString());
			direct_img.loadTexture("res/ui/GameRoomUI/" + this.directionList[(player.serverSitNum + i)%4] + ".png");
			// var light_direct_img = this.cur_player_panel.getChildByName("light_img" + i.toString()).getChildByName("direct_img");
			// light_direct_img.loadTexture("res/ui/GameRoomUI/gameroom_" + this.directionList[(player.serverSitNum + i)%4] + ".png");
		}
	...
	update_curplayer_panel:function(serverSitNum){
		...
		for(var i = 0; i < 4; i++){
			// var cur_selected_img = this.cur_player_panel.getChildByName(this.directionList[i] + "_bg");
	...
	update_player_info_panel:function(serverSitNum, playerInfo){
		...
		var yellow_frame_img = ccui.helper.seekWidgetByName(cur_player_info_panel, "yellow_frame_img");
		cur_player_info_panel.reorderChild(yellow_frame_img, 2);
		...
		cur_player_info_panel.reorderChild(dealer_img, 3);
		...
		cur_player_info_panel.reorderChild(owner_img, 4);
		if(serverSitNum == 0){
	...
	startBeginAnim:function(diceList, dealerIdx){
		...
		function setBeginGameShow(is_show){
			...
			for (var i = 0; i < 4; i++) {
				self.rootUINode.getChildByName("draw_tile_panel" + i).setVisible(is_show)
			}
			...
			for (var i = 0; i < 4; i++) {
				var cur_player_tile_panel = self.rootUINode.getChildByName("player_tile_panel" + i.toString()).getChildByName("player_hand_panel");
  ...
  //   	var curPlayNum = 0
		...
		for (var i = 0; i < diceList.length; i++) {
			if (diceList[i][0] > 0 && diceList[i][1] > 0) {
				playDiceEffect(h1global.entityManager.player().server2CurSitNum(i%4), diceList[i])
			}
		}
		...
		if(idx == 0){
			...
			for(var i = 0; i < 14; i++){
				...
				if(handTilesList[i]){
					...
					if(player.curGameRoom.kingTile == handTilesList[i]){
	...
	init_operation_panel:function(){
			...
			function(sender, eventType){
				if(eventType == ccui.Widget.TOUCH_ENDED){
					if(self.operation_type){
						player.confirmOperation(const_val.OP_PASS, [player.curGameRoom.lastDiscardTile]);
					} else {
		...
		this.operation_panel.getChildByName("op_btn7").addTouchEventListener(
			function(sender, eventType){
				if(eventType == ccui.Widget.TOUCH_ENDED){
					if(self.operation_type){
						player.confirmOperation(const_val.OP_WIN, [player.curGameRoom.lastDiscardTile]);
					} else {
						player.doOperation(const_val.OP_WIN, [0]);
					}
					self.hide_operation_panel();
				}
			});
		...
		if(player.curGameRoom.waitAid >= 0){
			...
			// 是否可以胡
			if(player.canWin(player.curGameRoom.handTilesList[player.serverSitNum], 0)){
				cc.log("test:333333333333333333")
	...
	defaultKongSelected:function(){
		...
		if(player.canSelfExposedKong(player.curGameRoom.upTilesList[player.serverSitNum], handTiles[lastHandTileIdx])){
	...
	playOperationEffect:function(opId, serverSitNum){
		var curSitNum = -1;
	...
	update_draw_tile_panel:function(leftTileNum){
		var hasDrawTileNum = 112 - leftTileNum;
		var player = h1global.entityManager.player();
		var fromIdx = (player.curGameRoom.dealerIdx + 2) % 4;
		for(var i = 0; i < 4; i++){
			var draw_tile_panel = this.rootUINode.getChildByName("draw_tile_panel" + (player.server2CurSitNum((fromIdx - i + 4) % 4)).toString());
			for(var j = 0; j < 28; j++){
	...
	update_kingtile_panel:function(){
		...
		var mahjong_img = tile_img.getChildByName("mahjong_img");
		mahjong_img.ignoreContentAdaptWithSize(true);
		if (player.curGameRoom.kingTile > 0) {
			mahjong_img.loadTexture("Mahjong/mahjong_big_" + player.curGameRoom.kingTile.toString() + ".png", ccui.Widget.PLIST_TEXTURE);
			mahjong_img.setVisible(true);
		}
	...
	show_yellow_frame: function(waitidx){
		...
		for(var i = 0 ;i< 4;i++){
			var player_info_panel = self.rootUINode.getChildByName("player_info_panel" +i);
